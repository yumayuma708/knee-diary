...
interface KneeRecordDao {
    ...
    @Query("SELECT * FROM knee_record ORDER BY dateTime DESC") // 一番最新のものが一番最初にくるような順番でデータを取得する。このSQLを書くことでRoomが勝手にデータを取得してこの順番で表示してくれる。データに追加や更新があった場合も、自動で更新してくれる。
